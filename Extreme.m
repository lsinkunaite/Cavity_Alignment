function [Domain_From,Domain_To]=Extreme(input_matrix,input_matrix2,pks,locs,fraction)
% Defines boundaries of one cycle.
    ...
    while (Counter == 0)
        if ((((1-fraction)*180)<(abs(locs(Diff_locs(Iterator))-Max_loc1))) && ((abs(locs(Diff_locs(Iterator))-Max_loc1))<((1+fraction)*180)))
