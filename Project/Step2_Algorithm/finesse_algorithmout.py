#!/usr/bin/env python
"""-----------------------------------------------------------------
  Python file for plotting Finesse ouput finesse_algorithmout.out
  created automatically Tue Sep  8 11:53:57 2015

  Run from command line as: python finesse_algorithmout.py
  Load from python script as: import finesse_algorithmout
  And then use:
  finesse_algorithmout.run() for plotting only
  x,y=finesse_algorithmout.run() for plotting and loading the data
  x,y=finesse_algorithmout.run(1) for only loading the data
-----------------------------------------------------------------"""
...
def run(noplot=None):
	data = np.loadtxt('finesse_algorithmout.out',comments='%')
	rows,cols=data.shape
	x=data[:,0]
	y=data[:,1:cols]
	mytitle='finesse_algorithmout                Tue Sep  8 11:53:57 2015'
	if (noplot==None):
		...
		trace1=ax1.plot(x, y[:,0], '-', color = next(colorcycler), label = ' n3 : ')
		traces = trace1
		traces_a = traces
